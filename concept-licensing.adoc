...
[role="lead"]
Cloud Volumes ONTAP にはいくつかのライセンス モデルがあります。各オプションを使用すると、お客様のニーズを満たす消費モデルを選択できます。

ライセンスやサブスクリプションの購入または更新は、クラウドマーケットプレイスまたは NetApp （Bring-Your-Own License または BYOL）を通じて行うことができます。マーケットプレイスでは、年間サブスクリプションと従量課金制（PAYGO）オプションをご利用いただけます。詳細については、link:concept-license-management.html["Cloud Volumes ONTAPの容量ベースのライセンスの詳細"] を参照してください。

BYOLライセンスは NetApp から購入でき、あらゆるクラウド プロバイダに使用できます。BYOLライセンスの利用可能性に関する詳細については、 https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/whats-new.html#restricted-availability-of-byol-licensing-for-cloud-volumes-ontap["Cloud Volumes ONTAPの BYOL ライセンスの利用制限"^]を参照してください。
...
新規顧客は、容量ベースの年間マーケットプレイスサブスクリプションまたは Keystone サブスクリプションによる課金を選択できます。プライベートモードなど、一部のデプロイメントでは、前世代のノードベースの BYOL ライセンスも利用できます。

容量ベースのライセンス:: プロビジョニングされた容量に応じて、 NetAppアカウント内の複数のCloud Volumes ONTAPシステムの料金を支払います。アドオンのクラウド データ サービスを購入する機能が含まれます。容量ベースのライセンスの消費モデルや購入オプションの詳細については、以下を参照してください。link:concept-licensing-charging.html["容量ベースのライセンスについて詳しく見る"] 。
...
* link:reference-configs-azure.html["Azureでサポートされている構成"]
...
* link:reference-configs-gcp.html["Google Cloud でサポートされている構成"^]
...
NOTE: バックアップとリカバリには従量課金制（PAYGO）のサブスクリプションが必要ですが、このサービスの利用に料金は発生しません。バックアップとリカバリのライセンス設定の詳細については、 https://docs.netapp.com/us-en/data-services-backup-recovery/task-licensing-cloud-backup.html["バックアップとリカバリのライセンスを設定する"^]を参照してください。
...
https://docs.netapp.com/us-en/console-setup-admin/concept-connectors.html#impact-on-cloud-volumes-ontap["コンソールエージェントについて学ぶ"^]。


NOTE: NetApp は、BYOLライセンスの購入、延長、更新を制限しました。詳細については、 https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/whats-new.html#restricted-availability-of-byol-licensing-for-cloud-volumes-ontap["Cloud Volumes ONTAPPの BYOL ライセンスの利用制限"^]を参照してください。
...
NOTE: プライベートモードでの Cloud Volumes ONTAP のプライベート展開。
...
| ライセンス タイプ | EOA後の影響 
 ...
 a| 
NetApp から購入した有効なノードベースライセンス、または BYOL。
 a| 
...
このライセンスを使用して新しいCloud Volumes ONTAPシステムを展開する権利はありません。既存のシステムは引き続き動作する可能性がありますが、EOS 日以降はシステムに対するサポートやアップデートは受けられなくなります。
 ...
 a| 
有効なノードベースライセンスと従量課金制サブスクリプション。
 a| 
EOS 日以降は、容量ベースのライセンスに移行するまで、 NetApp のサポートを受けられなくなります。

|===
.例外
NetApp は、AWS における Cloud Volumes ONTAP の中国リージョン展開など、特定の状況では特別な考慮が必要であることを認識しており、ノードベースのライセンスの EOA および EOS は以下のケースには適用されない場合があります：
...
コンソールを使用すると、ライセンス変換ツールを通じて、ノードベースのライセンスを容量ベースのライセンスにシームレスに変換できます。ノードベースライセンスのEOAに関する情報については、link:concept-licensing.html#end-of-availability-of-node-based-licenses["ノードベースライセンスの提供終了"]を参照してください。
...
##マーケットプレイスのサブスクリプションまたはノードベースのライセンスを使用したプライベートデプロイメントがある場合は、Console のライセンス変換ツールを使用してlink:task-convert-node-capacity.html["ノードベースのライセンスを容量ベースのライセンスに変換します"]できます。##
...
.関連リンク
* link:concept-licensing-charging.html["容量ベースのライセンスについて詳しく見る"]
