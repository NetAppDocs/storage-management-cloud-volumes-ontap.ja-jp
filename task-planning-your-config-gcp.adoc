...
機械の種類:: サポートされているマシンタイプをlink:reference-configs-gcp.html["Cloud Volumes ONTAPリリースノート"^]で確認し、各サポート対象マシンタイプの詳細についてはGoogleの情報をご参照ください。ワークロードの要件に合わせて、マシンタイプのvCPU数とメモリ容量を選択してください。各CPUコアはネットワークパフォーマンスを向上させることに注意してください。
+
--
詳細については、以下を参照してください。
...
* link:reference-configs-gcp.html#google-cloud-single-node["Google Cloud のCloud Volumes ONTAPシステムデータのデフォルト ディスクを表示する"]。
...
TIP: コンソールエージェントにはシステムディスクも必要です。 https://docs.netapp.com/us-en/console-setup-admin/reference-connector-default-config.html["コンソールエージェントのデフォルト構成の詳細を表示する"^]
