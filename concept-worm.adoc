...
WORM機能は、マーケットプレイスのサブスクリプションおよびBYOL（Bring-Your-Own License）で追加料金なしでご利用いただけます。現在のライセンスにWORMを追加するには、NetAppの営業担当者にお問い合わせください。
...
NFS または CIFS を介して WORM にファイルをコミットするにはアプリケーションを使用するか、 ONTAP CLI を使用してファイルを WORM に自動的にコミットします。WORM 追記可能ファイルを使用すると、ログ情報のように増分的に書き込まれるデータを保持することもできます。詳細については、 https://docs.netapp.com/us-en/ontap/snaplock/commit-files-worm-state-manual-task.html#commit-files-to-worm-manually["ONTAP SnapLockを使用してファイルをWORMにコミット"^]を参照してください。

Cloud Volumes ONTAP システムで WORM ストレージを有効化した後は、WORM ストレージのすべての管理に ONTAP CLI を使用する必要があります。手順については、link:https://docs.netapp.com/us-en/ontap/snaplock/index.html["SnapLockに関するONTAPドキュメント"^]を参照してください。
