...
* iSCSIボリュームを作成すると、コンソールが自動的にLUNを作成します。ボリュームごとに1つのLUNを作成するだけで済むように簡素化したので、管理作業は一切不要です。ボリュームを作成したら、link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-connect-lun.html["IQNを使用してホストからLUNに接続します"^]。
* ONTAP System Manager またはONTAP CLI から追加の LUN を作成できます。


ifdef::aws[]

* AWSでCIFSを使用するには、DNSとActive Directoryの設定が必要です。詳細については、link:reference-networking-aws.html["AWS におけるCloud Volumes ONTAPのネットワーク要件"]を参照してください。
...
.特定の骨材
--
.. 「アグリゲート」タブで、必要なアグリゲートに移動してimage:icon-action.png["追加操作のための3つの横長の点を示すアイコン"]アイコンをクリックします。
...
NTPサーバ:: Active Directory DNSを使用してNTPサーバーを設定するには、*Active Directoryドメインを使用する*を選択してください。別のアドレスを使用してNTPサーバーを設定する必要がある場合は、APIを使用してください。詳細については、 https://docs.netapp.com/us-en/console-automation/index.html["NetApp Console自動化に関するドキュメント"^]を参照してください。
+
--
...
+
*** link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-planning-your-config.html#size-your-system-in-aws["AWS でのシステムのサイズ設定"^]
...
* link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-planning-your-config-azure.html#size-your-system-in-azure["Azure でのシステムのサイズ設定"^]
...
* link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-planning-your-config-gcp.html#size-your-system-in-gcp["Google Cloud でのシステムのサイズ設定"^]
