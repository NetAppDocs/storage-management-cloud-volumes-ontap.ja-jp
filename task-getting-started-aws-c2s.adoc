...
summary: Cloud Volumes ONTAP を AWS Secret Cloud および Top Secret Cloud 環境に導入できます。AWS Secret Cloud および Top Secret Cloud リージョンは、米国諜報機関専用の非公開リージョンです。このページに記載されている手順は、AWS Secret Cloud および Top Secret Cloud リージョンのユーザーにのみ適用されます。 
---
...
+
プライベートモードの仕組みについて詳しくは、link:https://docs.netapp.com/us-en/console-setup-admin/concept-modes.html#private-mode["コンソールのプライベート展開モード"^]を参照してください。
...
. Amazon Simple Storage Service（Amazon S3）サービスへのVPCエンドポイントを設定します。
+
