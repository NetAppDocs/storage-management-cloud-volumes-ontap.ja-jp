...
[role="lead"]
既存の Cloud Volumes ONTAP システムを検出してNetApp Console に追加し、一元管理できます。アカウントを使用してシステムをオンボードすると、システムはそのアカウントに登録されます。複数のアカウントまたは組織がある環境では、Console ログインアカウントに登録されているシステムのみを検出および管理できます。

システム登録を操作するときは、すべてのアクションが、システムが最初にオンボードされたのと同じ組織およびアカウント内で実行されるようにしてください。例えば、移行が同じ組織内に留まる場合、Cloud Volumes ONTAPシステムを新しいConsoleエージェントに移行できます。


CAUTION: 別のアカウントまたは組織に登録されているシステムを検出、表示、または管理することはできません。
