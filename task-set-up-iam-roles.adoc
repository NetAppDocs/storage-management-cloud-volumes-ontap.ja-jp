...
[role="lead"]
必要な権限を持つ AWS Identity and Access management (IAM) ロールを各Cloud Volumes ONTAPノードに添付する必要があります。HA メディエーターについても同様です。  NetAppコンソールで IAM ロールを作成するのが最も簡単ですが、独自のロールを使用することもできます。
...
+
Cloud Volumes ONTAPシステムでNetApp Backup and Recovery を使用する予定の場合は、ノードの IAM ロールに以下に示す 2 番目のポリシーを含める必要があります。
