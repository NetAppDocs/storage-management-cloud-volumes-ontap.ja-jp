...
---
= AzureのCloud Volumes ONTAPでSnapMirrorトラフィックを分離する
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
----
.. privateIPAddressを保存します。このIPアドレス<new_added_nic_primary_addr>は、<<step-second,新しいNICのブロードキャストドメインとクラスタ間LIF>>でクラスタ間LIFを作成するために使用されます。
...
== [[step-second]]ステップ2：新しいNIC用に新しいIPspace、ブロードキャストドメイン、およびクラスタ間LIFを作成します
