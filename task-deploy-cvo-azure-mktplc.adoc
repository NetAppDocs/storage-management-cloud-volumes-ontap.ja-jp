...
* Cloud Volumes ONTAP の無料ライセンスは、最大 500 GiB のプロビジョニング容量に対して提供されており、NetApp サポートはなく、有効期限もありません。500 GiB を超える容量を使用するには、有効なライセンス権限が必要となり、料金が発生する場合があります。Cloud Volumes ONTAP のフルエクスペリエンスと高度な管理機能にアクセスするには、Cloud Volumes ONTAP システムを NetApp Console に接続してください。NetApp Console を通じた検出と管理には、お客様の NetApp Console アカウントに関連付けられた有効な Azure Marketplace サブスクリプションが必要です。
...
.開始する前に
* デプロイされたシステムをNetApp Consoleで検出して管理する予定がある場合は、NetApp Consoleアカウントが有効なAzure Marketplaceサブスクリプションに関連付けられていることを確認してください。NetApp Consoleへのシステムの検出には、有効なサブスクリプションが必要です。
