...
---
= Cloud Volumes ONTAP の書き込み速度について
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
単一ノードシステムを使用する場合、Cloud Volumes ONTAPはすべてのインスタンス タイプで高速書き込み速度をサポートします。

9.8 リリース以降、 Cloud Volumes ONTAP は、 m5.xlarge と r5.xlarge を除く、サポートされているほぼすべての EC2 インスタンス タイプを使用する場合、HA ペアによる高速書き込みをサポートします。

link:reference-configs-aws.html["Cloud Volumes ONTAPがサポートする Amazon EC2 インスタンスの詳細"]。
...
単一ノードシステムを使用する場合、Cloud Volumes ONTAPはすべてのVMタイプで高速書き込み速度をサポートします。

HAペアを使用する場合、Cloud Volumes ONTAPはバージョン9.8以降、複数のVMタイプで高速書き込みをサポートしています。高速書き込みをサポートするVMタイプを確認するには、link:reference-configs-azure.html["Cloud Volumes ONTAPリリースノート"]を参照してください。
...
単一ノードシステムを使用する場合、Cloud Volumes ONTAPはすべてのマシン タイプで高速書き込み速度をサポートします。

HAペアを使用する場合、Cloud Volumes ONTAPはバージョン9.13.0以降、複数のVMタイプで高速書き込み速度をサポートします。高速書き込み速度をサポートするVMタイプを確認するには、link:reference-configs-gcp.html#supported-configurations-by-license["Cloud Volumes ONTAPリリースノート"^]を参照してください。

link:reference-configs-gcp.html["Cloud Volumes ONTAPがサポートする Google Cloud マシンタイプの詳細"^]。
