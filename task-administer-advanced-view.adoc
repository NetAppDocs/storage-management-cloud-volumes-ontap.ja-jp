...
* ONTAP S3 オブジェクト ストレージ管理：Cloud Volumes ONTAP の ONTAP S3 ストレージ管理機能は System Manager でのみ使用でき、NetApp Console では使用できません。
...
. 必要なコンソールエージェントの image:icon-action.png["追加操作のための3つの横長の点を示すアイコン"] アイコンをクリックし、「コンソールエージェントの編集」を選択します。
...
* https://docs.netapp.com/us-en/ontap/concept_administration_overview.html["ONTAP System Managerを使用したクラスタ管理について学習します"^]
