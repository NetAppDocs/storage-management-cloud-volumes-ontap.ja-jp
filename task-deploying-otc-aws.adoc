...
---
= AWSの拡張方式を使用したCloud Volumes ONTAPの導入
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
* 稼働中のコンソール エージェント。
+
**  https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-aws.html["システムに関連付けられたコンソールエージェント"^]が必要です。
...
. プロンプトが表示されたら、 https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-aws.html["コンソールエージェントを作成する"^]。
...
| 資格情報の編集 | このシステムをデプロイするアカウントに関連付けられているAWS認証情報を選択してください。このCloud Volumes ONTAPシステムで使用するAWS Marketplaceサブスクリプションを関連付けることもできます。「サブスクリプションの追加」をクリックして、選択した認証情報を新しいAWS Marketplaceサブスクリプションに関連付けます。サブスクリプションは、年間契約またはCloud Volumes ONTAPの時間単位の料金支払いに使用できます。https://docs.netapp.com/us-en/console-setup-admin/task-adding-aws-accounts.html["NetApp ConsoleにAWS認証情報を追加する方法を学びます"^]。 
|===
+
...
. *場所と接続性*： 記録したネットワーク情報を https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-planning-your-config.html#collect-networking-information["AWS ワークシート"^]に入力してください。
+
...
+
詳細については、link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-planning-your-config.html#choose-a-volume-usage-profile["ボリューム使用プロファイルの理解"^]、link:concept-data-tiering.html["データ階層化の概要"^]、および https://kb.netapp.com/Cloud/Cloud_Volumes_ONTAP/What_Inline_Storage_Efficiency_features_are_supported_with_CVO#["KB: CVO ではどのようなインライン ストレージ効率機能がサポートされていますか?"^]を参照してください。
...
| 資格情報の編集 | この Cloud Volumes ONTAP システムで使用する AWS 認証情報とマーケットプレイスサブスクリプションを選択してください。*サブスクリプションの追加* をクリックして、選択した認証情報を新しい AWS マーケットプレイスサブスクリプションに関連付けます。サブスクリプションは、年間契約または Cloud Volumes ONTAP を時間単位で支払うためのものです。ライセンスを NetApp から直接購入した場合（Bring-Your-Own License または BYOL）、AWS サブスクリプションは必要ありません。NetApp は BYOL ライセンスの購入、延長、および更新を制限しました。詳細については、 https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/whats-new.html#restricted-availability-of-byol-licensing-for-cloud-volumes-ontap["Cloud Volumes ONTAPの BYOL ライセンスの利用制限"^]を参照してください。https://docs.netapp.com/us-en/console-setup-admin/task-adding-aws-accounts.html["コンソールにAWS認証情報を追加する方法を学びます"^]。 
|===
+
...
+
** https://docs.netapp.com/us-en/data-services-data-classification/concept-cloud-compliance.html["NetApp Data Classificationの詳細"^]
** https://docs.netapp.com/us-en/data-services-backup-recovery/task-backup-to-s3.html["バックアップとリカバリの詳細"^]
+
...
+
IPアドレスは、リージョン内のすべてのVPCのCIDRブロックの範囲外である必要があります。詳細については、link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/reference-networking-aws.html#requirements-for-ha-pairs-in-multiple-azs["複数の AZ におけるCloud Volumes ONTAP HA の AWS ネットワーク要件"^]を参照してください。
...
. *使用プロファイル、ディスク タイプ、階層化ポリシー*: ストレージ効率機能を有効にするかどうかを選択し、必要に応じてボリューム階層化ポリシーを編集します。
+
詳細については、link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-planning-your-config.html#choose-a-volume-usage-profile["ボリューム使用プロファイルを選択する"^]およびlink:concept-data-tiering.html["データ階層化の概要"^]を参照してください。
...
.関連リンク
* link:task-onboard-cvo-aws-marketplace.html["AWS MarketplaceからCloud Volumes ONTAPをサブスクライブしてデプロイする"]
