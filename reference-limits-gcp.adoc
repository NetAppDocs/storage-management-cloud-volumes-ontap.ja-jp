...
一部の構成では、ディスク制限により、ディスクのみを使用して容量制限に到達できなくなります。容量制限に達するには https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/concept-data-tiering.html["非アクティブなデータをオブジェクトストレージに階層化する"^]。詳細については、以下のディスク制限を参照してください。
...
https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-managing-svms-gcp.html["追加のストレージ VM を作成する方法を学ぶ"^]。
...
https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/concept-ha-google-cloud.html["Google Cloud で実行される Cloud Volumes ONTAP HA ペアでのストレージの仕組みを学ぶ"^]。
