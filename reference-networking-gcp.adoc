...
summary: Google Cloud ネットワークを設定して、Cloud Volumes ONTAP システムが正常に動作できるようにします。CVO のネットワーク要件、プライベート IP アドレス、VPC、サブネット、パケットミラーリング、ファイアウォールルール、HA 構成、およびコンソールエージェントの要件について説明します。 
---
...
https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-google.html["コンソール エージェントのデプロイで説明されている必要な共有 VPC 権限を確認します。"^]
...
コンソールエージェントは、日常的な運用のため、複数のエンドポイントにも接続します。エンドポイントに関する情報については、 https://docs.netapp.com/us-en/console-setup-admin/task-install-connector-on-prem.html#step-3-set-up-networking["コンソールエージェントから接続されたエンドポイントを表示する"^]および https://docs.netapp.com/us-en/console-setup-admin/reference-networking-saas-console.html["コンソールを使用するためのネットワークの準備"^]を参照してください。
...
TIP: コンソールエージェントに関する情報をお探しですか？ https://docs.netapp.com/us-en/console-setup-admin/reference-ports-gcp.html["コンソールエージェントのファイアウォールルールを表示する"^]
...
=== プライベートモードまたは保護された環境でのインフラストラクチャマネージャの構成

プライベートモードまたは VPC Service Controls で保護されたプロジェクトに Cloud Volumes ONTAP 9.16.1 以降をデプロイする場合は、Google が最終的に廃止する予定の Deployment Manager の代わりに、Cloud Volumes ONTAP がデプロイメントサービスとして Google Cloud Infrastructure Manager を使用できるよう、いくつかの構成変更を行う必要があります。

.開始する前に
* Cloud Volumes ONTAPシステムが9.16.1以降であることを確認してください。そうでない場合は、システムをアップグレードしてください。手順については https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-updating-ontap-cloud.html["Cloud Volumes ONTAP のアップグレード"]を参照してください。
* Google Cloud API が有効になっていることを確認します。 https://docs.netapp.com/us-en/console-setup-admin/task-install-agent-google-gcloud.html#step-5-enable-google-cloud-apis["Google Cloud API を有効にする"^]を参照してください。
* Cloud Build API が有効になっていることを確認します。参照 https://console.cloud.google.com/flows/enableapi?apiid=cloudbuild.googleapis.com&redirect=https://console.cloud.google.com/build/docs/private-pools/set-up-private-pool-environment["ここで Cloud Build API を有効にします"^]。
* Console エージェントのサービスアカウントにすべての標準権限があることを確認します。さらに、サービスアカウントに `cloudbuild.workerpools.get`および `cloudbuild.workerpools.list`権限があることを確認します。 https://docs.netapp.com/us-en/console-setup-admin/reference-permissions-gcp.html#service-account-permissions["コンソール エージェントの Google Cloud 権限"^]を参照してください。


.手順
. Cloud Volumes ONTAPデプロイメントと同じリージョンにこの構成でプライベートワーカープールを作成します。プライベートワーカープールの作成については、 https://docs.cloud.google.com/build/docs/private-pools/create-manage-private-pools#creating_a_new_private_pool["Google Cloud ドキュメント：プライベート プールの作成と管理"^]および https://cloud.google.com/build/pricing["Google Cloud Build の料金"^]を参照してください。
+
VPC Service Controls で保護されているプロジェクトに Cloud Volumes ONTAP をデプロイする場合は、Console エージェントと同じプロジェクト内にプライベートワーカープールを作成してください。ワーカープールは稼働状態である必要があります。Google Cloud のデフォルトのパブリック Cloud Build ワーカープールは、サービス境界内で実行できません。
...
**  https://cloud.google.com/apis["Google Cloud API"^]にアクセスするように設定されたサブネット。サブネットが Google API にアクセスできることを確認するには、次の手順を実行します：
+
... サブネットに対して「プライベート Google アクセス」がオンになっていることを確認します。
... *VPC ネットワークレベル > プライベートサービスアクセスタブ > サービスに割り当てられた IP 範囲*に移動します。
... *IP 範囲の割り当て* を選択し、Google Compute Service へのプライベート接続用の内部 IP 範囲を割り当てます。
... *サービスへのプライベート接続*で、*接続の作成*を選択します。
... *Connected service producer = Google Cloud Platform* を選択します。
... 前の手順で作成したプライベート接続 IP 範囲の割り当てを指定します。




. このワーカープールを展開し、Cloud Volumes ONTAP管理のために実行し続けます。Google Cloudは、このワーカープールを使用して、すべてのTerraform操作を分離された環境で実行します。
. Cloud Volumes ONTAPをプライベートモードで導入する場合は、*GCPワーカープール*フィールドでこのワーカープールの名前を選択します。手順については、link:task-deploying-gcp.html["Google Cloud でCloud Volumes ONTAPを起動する"]を参照してください。
+
VPC Service Controls で保護されたプロジェクトに Cloud Volumes ONTAP を導入する場合は、API を使用して Cloud Volumes ONTAP システムを作成し、作成リクエスト本文に `workerPool`パラメータを含めてください。

+

NOTE: コンソール UI を使用して Cloud Volumes ONTAP をデプロイする場合、ワーカープールを含めることは現在サポートされていません。VPC Service Controls で保護されているプロジェクトでは、Cloud Volumes ONTAP をデプロイする唯一のサポート方法は API です。
...
* https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-google.html["コンソールエージェントのネットワーク要件を表示する"^]
...
Console エージェントのプロキシサーバーの設定方法については、 https://docs.netapp.com/us-en/console-setup-admin/task-configuring-proxy.html["プロキシサーバーを使用するようにコンソールエージェントを構成する"^]を参照してください。
