...
アップグレードできるCloud Volumes ONTAPのバージョンは、現在実行しているバージョンによって異なります。次の表のリリース内の各汎用バージョンまたはパッチバージョンは、アップグレード可能な基本バージョンを表します。利用可能なパッチの詳細については、各リリースの https://docs.netapp.com/us-en/cloud-volumes-ontap-relnotes/reference-new.html["バージョン付きリリースノート"^]を参照してください。

ifdef::aws[]



==== AWS でサポートされているアップグレードパス
...
==== Google Cloud でサポートされているアップグレード パス
...
このページに記載されているいずれかの方法を使用してソフトウェアをアップグレードするには、Cloud Volumes ONTAP を NetApp サポートに登録する必要があります。これは従量課金制（PAYGO）とライセンス持ち込み（BYOL）の両方に適用されます。link:task-registering.html["PAYGOシステムを手動で登録する"]が必要です。BYOL システムはデフォルトで登録されています。
...
link:https://docs.netapp.com/us-en/cloud-volumes-ontap-relnotes/reference-limits-aws.html#disk-and-tiering-limits-by-ec2-instance["EC2インスタンスごとにディスクと階層化の制限を確認する"^] link:https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-change-ec2-instance.html["Cloud Volumes ONTAPのEC2インスタンスタイプを変更する"^]
...
. 必要なアグリゲートタイルで、image:icon-action.png["追加操作のための3つの横長の点を示すアイコン"]アイコンをクリックし、*アグリゲートの詳細を表示*を選択します。
+
...
. システム上で、image:icon-action.png["追加操作のための3つの横長の点を示すアイコン"]アイコンをクリックし、次に「*Update Cloud Volumes ONTAP*」をクリックします。
