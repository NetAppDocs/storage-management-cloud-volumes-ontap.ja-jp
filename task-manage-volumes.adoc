...
.手順
. ONTAP System Manager でボリュームのサイズを変更するには、System Manager ビューを選択します。link:task-administer-advanced-view.html#get-started-with-system-manager["システムマネージャーを使い始める"]を参照してください。
...
TIP: コンソールは、アグリゲートの使用容量が90%に達すると、これらの「対応が必要です」というメッセージを表示します。データ階層化が有効になっている場合、アグリゲートの使用容量が80%に達したときにメッセージが表示されます。デフォルトでは、データ階層化のために10%の空き容量が確保されます。link:task-tiering.html#change-the-free-space-ratio-for-data-tiering["データ階層化の空き領域比率について詳しく見る"^]。
...
.. 集計タイルで、image:icon-action.png["追加操作のための3つの横長の点を示すアイコン"]アイコンをクリックし、次に「集計の詳細を表示」をクリックします。
...
+
詳細については、link:https://docs.netapp.com/us-en/data-services-replication/task-replicating-data.html["システム間でのデータの複製"^]を参照してください。

. 「レプリケーション ステータス」ページに移動し、 SnapMirror関係を解除して、複製されたボリュームをデータ保護ボリュームから読み取り/書き込みボリュームに変換します。
+
詳細については、link:https://docs.netapp.com/us-en/data-services-replication/task-replicating-data.html#managing-data-replication-schedules-and-relationships["データ複製スケジュールと関係の管理"^]を参照してください。
...
+
.. Cloud Volumes ONTAPシステムで、*Aggregates タブ*をクリックします。
.. 必要なアグリゲートタイルで、image:icon-action.png["追加操作のための3つの横長の点を示すアイコン"]アイコンをクリックし、*集計の詳細を表示* をクリックすると、使用可能な容量（プロビジョニングされたサイズから使用済みのアグリゲート容量を差し引いたもの）が表示されます。
+
...
+
.. アグリゲートを選択し、image:icon-action.png["追加操作のための3つの横長の点を示すアイコン"] アイコン > *ディスクの追加* をクリックします。
