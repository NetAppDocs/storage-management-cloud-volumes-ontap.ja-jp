...
ディスク:: 顧客データはプレミアムストレージページのブロブに保存されます。各ノードは、他のノードのストレージにアクセスできます。追加のストレージはlink:reference-configs-azure.html#azure-ha-pair["ブート、ルート、コアデータ"]にも必要です。
ストレージアカウント::
+
--
...
NOTE: Cloud Volumes ONTAP 9.15.0P1以降、Azureページblobは新しい高可用性ペアデプロイメントではサポートされなくなりました。現在、既存の高可用性ペアデプロイメントでAzureページblobを使用している場合は、Edsv4シリーズVMおよびEdsv5シリーズVMの新しいVMインスタンスタイプに移行できます。link:reference-configs-azure.html#ha-pairs["Azureでサポートされている構成の詳細"]。
...
ディスク:: 顧客データは、ローカル冗長ストレージ（LRS）で管理されるディスクに保存されます。各ノードは、他のノードのストレージにアクセスできます。追加のストレージはlink:reference-configs-azure.html#azure-ha-pair["ブート、ルート、パートナールート、コア、 NVRAMデータ"]にも必要です。
...
ディスク:: 顧客データは、ゾーン冗長ストレージ（ZRS）で管理されるディスクに保存されます。各ノードは、他のノードのストレージにアクセスできます。追加のストレージもlink:reference-configs-azure.html#azure-ha-pair["ブート、ルート、パートナールート、コアデータ"]に必要です。
ストレージアカウント:: ストレージ アカウントは、診断ログと BLOB ストレージへの階層化を処理するために、マネージド ディスク ベースの展開に使用されます。
...
NOTE: Cloud Volumes ONTAP は、プローブ用にアクティブ / パッシブ モードでロード バランサーを使用します。Azure では、アクティブ ノードのみがプローブに応答します。パッシブ ノードは、X マークが表示されて非アクティブ状態として認識される場合があります。これは想定される動作であり、心配する必要はありません。

.関連リンク
link:task-azure-high-availability-mode.html["AzureでCloud Volumes ONTAPの高可用性モードを有効にする"^]
