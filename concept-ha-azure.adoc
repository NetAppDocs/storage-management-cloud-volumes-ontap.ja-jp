...
NOTE: Cloud Volumes ONTAP 9.15.0P1以降、Azureページblobは新しい高可用性ペアデプロイメントではサポートされなくなりました。現在、既存の高可用性ペアデプロイメントでAzureページblobを使用している場合は、Edsv4シリーズVMおよびEdsv5シリーズVMの新しいVMインスタンスタイプに移行できます。link:link:reference-configs-azure.html#ha-pairs["Azureでサポートされている構成の詳細"]。
