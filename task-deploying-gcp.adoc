...
+
**  https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-google.html["システムに関連付けられたコンソールエージェント"^]が必要です。
** https://docs.netapp.com/us-en/console-setup-admin/concept-connectors.html["コンソールエージェントを常に実行しておく必要があります"^]。
** コンソールエージェントに関連付けられたサービスアカウント https://docs.netapp.com/us-en/console-setup-admin/reference-permissions-gcp.html["必要な権限を持っている必要があります"^]。
...
. プロンプトが表示されたら、 https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-google.html["コンソールエージェントを作成する"^]。
...
| サービスアカウント名 | Cloud Volumes ONTAP でlink:concept-data-tiering.html["データ階層化"]または https://docs.netapp.com/us-en/data-services-backup-recovery/concept-backup-to-cloud.html["NetApp Backup and Recovery"^]を使用する予定がある場合は、*Service Account* を有効にして、事前定義されたストレージ管理者ロールを持つサービスアカウントを選択する必要があります。link:task-creating-gcp-service-account.html["サービスアカウントの作成方法を学ぶ"^]。 
...
ドロップダウン リストに追加のプロジェクトが表示されない場合は、サービス アカウントが他のプロジェクトにまだ関連付けられていません。Google Cloud Console に移動し、IAM サービスを開いて、プロジェクトを選択します。NetApp Console に使用するロールを持つサービス アカウントをそのプロジェクトに追加します。プロジェクトごとにこの手順を繰り返す必要があります。


NOTE: これはコンソール用に設定したサービスアカウントです、link:https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-google.html["このページに記載されている通り"^]。

選択した資格情報をサブスクリプションに関連付けるには、「*サブスクリプションの追加*」をクリックします。

従量課金制の Cloud Volumes ONTAP システムを作成するには、Google Cloud Marketplace の Cloud Volumes ONTAP サブスクリプションに関連付けられた Google Cloud プロジェクトを選択する必要があります。 https://docs.netapp.com/us-en/console-setup-admin/task-adding-gcp-accounts.html["マーケットプレイスのサブスクリプションを Google Cloud 認証情報に関連付ける"^]を参照してください。
...
. *場所と接続*：システムの Google Cloud リージョンとゾーンを選択し、ファイアウォール ポリシーを選択して、データ階層化のための Google Cloud ストレージへのネットワーク接続を確認します。
+
次の表では、ガイダンスが必要になる可能性のあるフィールドについて説明します。
...
. *事前構成済みパッケージ*：パッケージの1つを選択してCloud Volumes ONTAPシステムを迅速に導入するか、*独自の構成を作成*をクリックします。事前構成済みパッケージは、選択したCloud Volumes ONTAPバージョンによって異なります。たとえば、Cloud Volumes ONTAP 9.18.1以降では、ConsoleにはHyperdisk Balancedディスクを含むC3 VMを含むパッケージが表示されます。ワークロードのニーズに応じて、IOPSやスループットパラメータなどの構成を変更できます。
+
...
+
ディスクの種類とサイズの選択については、link:task-planning-your-config-gcp.html#size-your-system-in-google-cloud["Google Cloud でシステムのサイズを決定する"^]を参照してください。

. *フラッシュキャッシュ、書き込み速度、WORM*:
+
.. 必要に応じて、*Flash Cache* を有効にするか、*標準* または *高* の書き込み速度を選択します。
+
link:reference-configs-gcp.html#flash-cache-support["Flash Cache"]およびlink:concept-write-speed.html["書き込み速度"]の詳細を確認してください。

+

NOTE: 「高速」書き込み速度オプションを選択すると、高速書き込み速度と、8,896バイトというより高い最大伝送単位（MTU）が利用可能になります。さらに、MTUが8,896と高いため、デプロイメントにはVPC-1、VPC-2、VPC-3を選択する必要があります。VPC-1、VPC-2、およびVPC-3の詳細については、 https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/reference-networking-gcp.html#requirements-for-the-console-agent["VPC-1、VPC-2、VPC-3のルール"^]を参照してください。
...
** Google Cloud アカウントの追加に関するヘルプについては、 https://docs.netapp.com/us-en/console-setup-admin/task-adding-gcp-accounts.html["9.6 でデータ階層化を行うための Google Cloud アカウントの設定と追加"^]を参照してください。
...
| NTPサーバ | Active Directory DNSを使用してNTPサーバーを設定するには、「Active Directoryドメインを使用する」を選択してください。別のアドレスを使用してNTPサーバーを設定する必要がある場合は、APIを使用してください。詳細については、 https://docs.netapp.com/us-en/console-automation/index.html["NetApp Console自動化ドキュメント"^]を参照してください。NTPサーバーの設定は、CIFSサーバーを作成する際にのみ可能です。CIFSサーバーを作成した後は、設定を変更することはできません。 
|===
...
CAUTION: 展開プロセスが完了したら、Google Cloud ポータル内のシステム生成のCloud Volumes ONTAP構成（システムタグやGoogle Cloud リソースに設定されたラベルなど）を変更しないでください。これらの構成に変更を加えると、予期しない動作やデータ損失が発生する可能性があります。
...
. *システムの詳細*：プロジェクトを選択し、クラスター名を指定し、必要に応じてサービスアカウントを選択し、必要に応じてラベルを追加し、最後に認証情報を指定します。
+
次の表では、ガイダンスが必要になる可能性のあるフィールドについて説明します。
...
| Google Cloudプロジェクトとマーケットプレイスのサブスクリプション  a| 
このシステムで使用する Google プロジェクトとサブスクリプションを選択してください。これは、コンソール用に設定したサービスアカウントです。詳細については、link:https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-connector-google.html["コンソール設定ドキュメント"^]を参照してください。コンソールはプロジェクトを使用して Cloud Volumes ONTAP システムを作成し、サブスクリプションを使用して請求と使用状況を決定します。ドロップダウンリストに他のプロジェクトが表示されない場合は、サービスアカウントがまだ他のプロジェクトに関連付けられていないことを意味します。Google Cloud Console にアクセスし、IAM サービスを開いて、プロジェクトを選択します。コンソールで使用するロールを持つサービスアカウントを、そのプロジェクトに追加してください。この手順はプロジェクトごとに繰り返す必要があります。

選択した資格情報をサブスクリプションに関連付けるには、「*サブスクリプションの追加*」をクリックします。

従量課金制の Cloud Volumes ONTAP システムを作成するには、Google Cloud Marketplace の Cloud Volumes ONTAP へのサブスクリプションに関連付けられた Google Cloud プロジェクトを選択する必要があります。コンソール設定ドキュメントを参照してください： https://docs.netapp.com/us-en/console-setup-admin/task-adding-gcp-accounts.html["マーケットプレイスのサブスクリプションを Google Cloud 認証情報に関連付ける"^]。
...
| サービスアカウント名 | link:concept-data-tiering.html["NetApp Cloud Tiering"]または https://docs.netapp.com/us-en/data-services-backup-recovery/concept-backup-to-cloud.html["バックアップとリカバリ"^]サービスを使用する場合は、*サービスアカウント*スイッチを有効にしてから、Storage Admin ロールが事前定義されているサービスアカウントを選択する必要があります。 


| ラベル | ラベルは、Google Cloud リソースのメタデータです。コンソールは、Cloud Volumes ONTAPシステムと、システムに関連付けられた Google Cloud リソースにラベルを追加します。システムを作成するときに、ユーザー インターフェイスから最大 4 つのラベルを追加でき、作成後にさらにラベルを追加できます。システムを作成するときに、API ではラベルが 4 つに制限されないことに注意してください。ラベルの詳細については、 https://cloud.google.com/compute/docs/labeling-resources["Google Cloud ドキュメント: ラベル付けリソース"^] 。 


| ONTAPユーザー名とパスワード | これらは、Cloud Volumes ONTAPクラスター管理者アカウントの資格情報です。これらの資格情報を使用して、 ONTAP System Manager またはONTAP CLI を介してCloud Volumes ONTAPに接続できます。デフォルトの _admin_ ユーザー名をそのまま使用するか、カスタム ユーザー名に変更します。 
|===
. *サービス*: このシステムで使用するサービスを選択します。バックアップとリカバリを選択するか、 NetApp Cloud Tieringを使用するには、手順 3 でサービス アカウントを指定する必要があります。
+
...
. *展開トポロジ*：HA構成には、複数の可用性ゾーン（AZ）（推奨）または単一のAZを選択します。
+
link:concept-ha-google-cloud.html["HA展開モデルの詳細"^] 。

. *ネットワーク構成と接続性*：該当するリージョンとゾーンを選択してください。HA構成用に4つの異なるVPCを選択し、各VPCにサブネットを1つずつ選択してから、ファイアウォールポリシーを選択します。
+

NOTE: Cloud Volumes ONTAP 9.19.1以降はlink:concept-ha-google-cloud.html["単一AZ HA構成用の共有ディスク"]をサポートしており、メディエーターインスタンスはオプションとなります。この構成では、データレプリケーションのために、メディエーターゾーン、対応するVPC、およびサブネットを入力する必要はありません。
...
+
いずれかのパッケージを選択した場合、必要なのはボリュームを指定し、構成を確認して承認するだけです。事前設定済みのパッケージは、選択した Cloud Volumes ONTAP のバージョンと構成タイプによって異なります。例えば、Cloud Volumes ONTAP 9.19.1 以降で単一の AZ デプロイメントを選択した場合、C3 シリーズのマシンと Hyperdisk Balanced ディスクを搭載した事前構成済みパッケージが表示されます。以前のバージョンや複数の AZ の場合でも、n2-standard マシンと Persistent Disks を選択できます。ワークロードのニーズに応じて、IOPS やスループットなどのパラメータを変更できます。
...
NOTE: n2-standard-16、n2-standard-32、n2-standard-48、およびn2-standard-64インスタンスタイプでは、*High*書き込み速度オプションを使用することで、高速な書き込み速度と8,896バイトというより高い最大伝送単位（MTU）を利用できます。さらに、MTUが8,896と高いため、デプロイメントにはVPC-1、VPC-2、VPC-3を選択する必要があります。高速書き込み速度と8,896のMTUは機能に依存するため、設定済みのインスタンス内で個別に無効にすることはできません。VPC-1、VPC-2、およびVPC-3の詳細については、 https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/reference-networking-gcp.html#requirements-for-the-connector["VPC-1、VPC-2、VPC-3のルール"^]を参照してください。
