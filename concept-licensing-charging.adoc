...
NOTE: NetApp は、BYOLライセンスの購入、延長、および更新を制限しており、プライベートモードでの展開など、特定のシナリオでの利用可能性を制限しています。詳細については、 https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/whats-new.html#restricted-availability-of-byol-licensing-for-cloud-volumes-ontap["Cloud Volumes ONTAPの BYOL ライセンスの利用制限"^]を参照してください。
...
* *マーケットプレイス サブスクリプション/年間契約*：クラウド プロバイダのマーケットプレイスからの年間契約。プライベート モードの展開では、マーケットプレイス サブスクリプションはサポートされていません。
...
価格の詳細については、link:https://www.netapp.com/pricing/["NetApp価格設定ウェブサイト"^]を参照してください。
...
容量ベースのライセンスでは、Cloud Volumes ONTAPシステムの最大数は、NetApp Console組織あたり24に制限されています。_システム_とは、Cloud Volumes ONTAP HAペア、Cloud Volumes ONTAP単一ノードシステム、または作成した追加のストレージVMのことです。デフォルトのストレージVMは制限にカウントされません。この制限はすべてのライセンスモデルに適用されます。
...
* link:task-convert-node-capacity.html["ノードベースのライセンスを容量ベースのライセンスに変換する"]
* link:task-manage-keystone.html["Keystoneサブスクリプションを管理する"]
