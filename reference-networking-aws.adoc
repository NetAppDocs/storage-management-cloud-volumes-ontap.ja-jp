...
Cloud Volumes ONTAP の導入時に、AWS メディエータープロキシのフィールドにはサーバーアドレスを指定できます。IP アドレスの代わりに完全修飾ドメイン名（FQDN）を入力した場合、AWS VPC にカスタム DNS サーバーが設定されていない限り、FQDN は解決されません。これは、AWS が提供するデフォルトの DNS はパブリック DNS レコードと AWS 内部ホストのみを解決し、顧客のプロキシ サーバなどの外部 FQDN を解決できないためです。この状況が該当する場合、選択肢は 2 つあります（

* FQDN の代わりに、メディエータープロキシフィールドにプロキシ サーバの IP アドレスを直接追加します。
* カスタムDNSサーバアドレスを含むDHCPオプションセットをVPCで設定します。これにより、AWSは、導入時にプロキシ サーバフィールドに入力したFQDNを解決できます。詳細については、 https://docs.aws.amazon.com/vpc/latest/userguide/DHCPOptionSet.html["AWSのドキュメント"^]を参照してください。
...
TIP: コンソールエージェントに関する情報をお探しですか？ https://docs.netapp.com/us-en/console-setup-admin/reference-ports-aws.html["コンソールエージェントのセキュリティグループルールを表示する"^]
...
EBS をパフォーマンス層として、Amazon S3 を容量層として使用する場合は、Cloud Volumes ONTAP が S3 に接続していることを確認する必要があります。この接続を提供する最善の方法は、S3 サービスへの VPC エンドポイントを作成することです。手順については、 https://docs.aws.amazon.com/AmazonVPC/latest/UserGuide/vpce-gateway.html#create-gateway-endpoint["AWS ドキュメント: ゲートウェイエンドポイントの作成"^]を参照してください。
...
* https://docs.netapp.com/us-en/console-setup-admin/concept-install-options-aws.html["コンソールエージェントのネットワーク要件を表示する"^]
