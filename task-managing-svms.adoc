...
[role="lead"]
ストレージVM（SVM）は、ONTAP内で実行される仮想マシンであり、クライアントにストレージおよびデータサービスを提供します。これは_SVM_または_vserver_として知られている場合があります。Cloud Volumes ONTAPはデフォルトで1つのストレージVMで構成されていますが、一部の構成では追加のストレージVMがサポートされています。
