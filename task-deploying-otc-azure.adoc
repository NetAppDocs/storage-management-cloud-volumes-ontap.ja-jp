...
+
このオプションは、特定のVMタイプでのみ利用可能です。サポートされているVMタイプを確認するには、link:link:reference-configs-azure.html#ha-pairs["HAペアのライセンスでサポートされる構成"]を参照してください。
...
+
link:link:reference-configs-azure.html#ha-pairs["Azureでサポートされている構成の詳細"^]。
